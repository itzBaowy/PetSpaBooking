...
import { useAdminBookingList } from "../queries";
...
export function BookingMonitoringPage() {
  const bookings = useAdminBookingList();
  const disputedBookings = bookings.data.filter((booking) => booking.disputeStatus === "OPEN").length;
  const totalRevenue = bookings.data.reduce((sum, booking) => sum + booking.amount, 0);
    ...
    <div className="mx-auto max-w-[1600px] space-y-6 p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-gray-500">
            Monitor every platform booking and identify cases needing admin intervention.
          </p>
        </div>
        <a
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-lg border border-gray-100 bg-white p-5 shadow-sm">
          <p className="text-sm font-medium text-gray-500">Total bookings shown</p>
          <p className="mt-2 text-3xl font-bold text-gray-900">{bookings.data.length}</p>
        </div>
        <div className="rounded-lg border border-gray-100 bg-white p-5 shadow-sm">
          <p className="text-sm font-medium text-gray-500">Open disputes</p>
          <p className="mt-2 text-3xl font-bold text-red-600">{disputedBookings}</p>
        </div>
        <div className="rounded-lg border border-gray-100 bg-white p-5 shadow-sm">
      ...
      <div className="rounded-lg border border-gray-100 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-gray-100 p-5 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="text-sm font-medium text-gray-500">Showing 1-3 of 3 bookings</div>
        </div>
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {bookings.data.map((booking) => (
                <tr key={booking.id}>
                  ...
                  <td className="px-5 py-4 text-gray-700">{booking.petOwner}</td>
                  <td className="px-5 py-4 text-gray-700">{booking.provider}</td>
                  <td className="px-5 py-4 text-gray-700">{booking.scheduledAt}</td>
                  <td className="px-5 py-4">
                    <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${paymentStatusStyles[booking.paymentStatus]}`}>
                      {booking.paymentStatus}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${bookingStatusStyles[booking.status]}`}>
                      {booking.status}
                    </span>
                  </td>
                  <td className="px-5 py-4">
