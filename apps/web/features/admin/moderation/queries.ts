...
export type ModerationStatus = "PENDING" | "APPROVED" | "HIDDEN" | "NEEDS_REVISION";
...
export const adminModerationKeys = {
  ...
  contents: () => [...adminModerationKeys.all, "contents"] as const,
  reports: () => [...adminModerationKeys.all, "reports"] as const,
};

export function useModerationQueue() {
  return useQuery<ModerationItem[]>({
    queryKey: adminModerationKeys.contents(),
    queryFn: async () => {
      const response = await api.get<ModerationItem[]>("/admin/moderation/contents");
...
export function useUserReports() {
  return useQuery<UserReport[]>({
    queryKey: adminModerationKeys.reports(),
    queryFn: async () => {
...
export function useResolveModerationContent() {
  return useMutation({
    mutationFn: async (payload: ContentModerationActionData) => {
      const response = await api.post("/admin/moderation/contents/resolve", payload);
...
export function useResolveUserReport() {
  return useMutation({
    mutationFn: async (payload: ReportResolutionData) => {
      const response = await api.post("/admin/moderation/reports/resolve", payload);
      return response.data as { success: boolean };
    },
  });
}
