  ...
  return (
    <div className="space-y-5">
      <ProviderPageHeader
        ...
        action={<Button onClick={() => setWithdrawOpen(true)}>Tạo yêu cầu rút tiền</Button>}
      />
      <div className="grid gap-4 sm:grid-cols-3">
        <Metric label="Số dư ví" value={providerMoney.format(wallet.walletBalance)} />
        <Metric label="Số dư ký quỹ" value={providerMoney.format(wallet.depositBalance)} />
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <LinkButton href="/provider/wallet/withdrawals" text="Xem yêu cầu rút tiền" />
      </div>
      {withdrawOpen ? (
        <ProviderWithdrawalDialog walletBalance={wallet.walletBalance} onClose={() => setWithdrawOpen(false)} />
      ) : null}
    </div>
  );
}
...
export function ProviderTransactionsPage() {
  ...
  return (
    <div className="space-y-5">
      <ProviderPageHeader title="Lịch sử giao dịch" description="Sổ giao dịch đọc từ Wallet API." />
      {result.items.length === 0 ? (
        <ProviderEmpty text="Chưa có giao dịch." />
      ) : (
        <div className="grid gap-3">
          {result.items.map((item) => (
            <article className="rounded-2xl border border-border-subtle bg-surface p-4 shadow-sm" key={item.id}>
              <div className="flex justify-between gap-4">
                <div>
                  <ProviderBadge value={item.type} />
                  <p className="mt-2 text-sm text-muted">{item.note ?? item.referenceId ?? item.id}</p>
                  <p className="text-xs text-muted">{providerDate(item.createAt)}</p>
                </div>
                <strong className={item.amount >= 0 ? "text-emerald-700" : "text-red-700"}>
      ...
      <Pager page={page} total={result.pagination.totalPages} setPage={setPage} />
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      <ProviderPageHeader
        ...
        action={<Button onClick={() => setCreateOpen(true)}>Tạo yêu cầu</Button>}
      />
      {result.items.length === 0 ? (
            ...
            <article className="rounded-2xl border border-border-subtle bg-surface p-4 shadow-sm" key={item.id}>
              <div className="flex justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-muted">{providerDate(item.requestedAt)}</p>
      ...
      <Pager page={page} total={result.pagination.totalPages} setPage={setPage} />
      {createOpen ? (
        <ProviderWithdrawalDialog walletBalance={wallet.data?.walletBalance} onClose={() => setCreateOpen(false)} />
      ) : null}
    </div>
  );
}
